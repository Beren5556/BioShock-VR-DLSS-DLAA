...
#include "bvr-sr-quality.h"

#include <nvsdk_ngx.h>
#include <nvsdk_ngx_helpers.h>
#if !defined(BVR_DLSS45_ONLY)
...
#if defined(BVR_DLSS45_ONLY)
static const char kProductIdentity[] =
    "BioShock VR DLSS 4.5 Complemento v1.0.0-rc.1 - By Beren5556";
static const char kNgxProjectId[] = "76d9b214-86fa-4650-9706-7b97b9482ad8";
static const char kNgxEngineVersion[] = "1.0.0-rc.1";
#else
static const char kProductIdentity[] = "DLSS5-Feeder host";
// This is a project-owned UUID, not an NVIDIA-issued application ID.
static const char kNgxProjectId[] = "76d9b214-86fa-4650-9706-7b97b9482ad8";
static const char kNgxEngineVersion[] = "0.14.0";
#endif
// BioShock VR phase 1 is a clean DLSS 4.5 SR/DLAA consumer. `--eye` selects
// that lane: no Neural Rendering add-on discovery, no RenoDX warm-up recreate,
...
// Keep one previous host log and cap the active one. BioShock runs one helper
// per eye, with a distinct path for each, so rotation never crosses histories.
// pacetrace.log is owned by the 32-bit VR mod and is deliberately not touched
// here: rotating another process' open file from either eye would be racy.
static const unsigned long long kMaxHostLogBytes = 4ull * 1024ull * 1024ull;

static bool PreviousLogPath(const char *log_path, char *out, size_t out_size)
{
    if (log_path == nullptr || log_path[0] == '\0' || strlen(log_path) + 6 >= out_size) return false;
    strcpy_s(out, out_size, log_path);
    char *ext = strrchr(out, '.');
    if (ext != nullptr && _stricmp(ext, ".log") == 0)
        strcpy_s(ext, out_size - static_cast<size_t>(ext - out), ".prev.log");
    else
        strcat_s(out, out_size, ".prev");
    return true;
}

static void RotateHostLog(const char *log_path, bool always)
{
    WIN32_FILE_ATTRIBUTE_DATA attrs = {};
    if (!GetFileAttributesExA(log_path, GetFileExInfoStandard, &attrs)) return;
    const unsigned long long size =
        (static_cast<unsigned long long>(attrs.nFileSizeHigh) << 32) | attrs.nFileSizeLow;
    if (!always && size < kMaxHostLogBytes) return;

    char previous[MAX_PATH] = {};
    if (!PreviousLogPath(log_path, previous, sizeof(previous))) return;
    MoveFileExA(log_path, previous, MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH);
}
...
static void Log(const char *fmt, ...)
{
    ...
    RotateHostLog(g_log_path, false);
    FILE *f = nullptr;
...
static bool InitDisguise()
{
    ...
    if (g_bvr_dlss45_mode)
        Log("[host] BioShock VR D3D12 helper device ready (eye %d)", g_bvr_eye);
    else
    {
...
static bool InitNgx()
{
    ...
    GetModuleFileNameW(nullptr, module_dir, MAX_PATH);
    if (wchar_t *s = wcsrchr(module_dir, L'\\')) *(s + 1) = L'\0';

    wchar_t data_path[MAX_PATH] = {};
    wcscpy_s(data_path, module_dir);
    if (g_bvr_dlss45_mode)
    {
        ...
        if (GetEnvironmentVariableW(L"LOCALAPPDATA", local, MAX_PATH) > 0)
        {
            ...
            _snwprintf_s(root, _TRUNCATE, L"%s\\BioshockVR", local);
            CreateDirectoryW(root, nullptr);
            _snwprintf_s(root, _TRUNCATE, L"%s\\BioshockVR\\DLSS45", local);
            CreateDirectoryW(root, nullptr);
            _snwprintf_s(data_path, _TRUNCATE, L"%s\\BioshockVR\\DLSS45\\eye%d\\",
                         local, g_bvr_eye);
    ...
    {
        char dir8[MAX_PATH] = {};
        WideCharToMultiByte(CP_UTF8, 0, module_dir, -1, dir8, MAX_PATH, nullptr, nullptr);
        FeedLogNgxRuntimes(&Log, "host", dir8);
    }
    else
        Log("[host] DLSS 4.5 runtime search path: %lsnvngx_dlss.dll", module_dir);
    ...
    // Keep the signed runtime next to the executable, but give each eye a
    // private writable NGX cache/log directory.
    const wchar_t *const search[2] = { module_dir, data_path };
    NVSDK_NGX_FeatureCommonInfo info = {};
    info.PathListInfo.Path   = search;
    info.PathListInfo.Length =
        _wcsicmp(module_dir, data_path) == 0 ? 1u : static_cast<unsigned>(_countof(search));
    ...
    // The generic feeder diagnostic also probes the later Neural Rendering
    // feature.  Do not mix that phase into BioShock VR's DLSS 4.5-only log.
    if (!g_bvr_dlss45_mode) NgxAskWhy(data_path, &info);

    // NVIDIA did not issue this project an application ID. Use the SDK's
    // documented custom-engine path with our own UUID and the API version from
    // these local NGX headers (NVSDK_NGX_VERSION_API_MACRO = 0x15 / API 1.5).
    NVSDK_NGX_Result r = NVSDK_NGX_D3D12_Init_with_ProjectID(
        kNgxProjectId, NVSDK_NGX_ENGINE_TYPE_CUSTOM, kNgxEngineVersion,
        data_path, h.dev, &info, NVSDK_NGX_Version_API);
    Log("[host] NGX Init_with_ProjectID (CUSTOM, project %s, engine %s, API 0x%X) -> 0x%08X (%s)",
        kNgxProjectId, kNgxEngineVersion, static_cast<unsigned>(NVSDK_NGX_Version_API),
        r, NgxResultName(r));
...
#if defined(BVR_DLSS45_ONLY)
        static const wchar_t *kMods[] = { L"_nvngx.dll", L"nvngx.dll", L"nvngx_dlss.dll" };
#else
    ...
    {
        int avail = 0, needs_driver = 0, maj = 0, min_v = 0;
...
#if defined(BVR_DLSS45_ONLY)
        Log("[host] NGX capabilities: SuperSampling.Available=%d NeedsUpdatedDriver=%d "
            "MinDriver=%d.%d (NR capability queries disabled in DLSS45-only build)",
            avail, needs_driver, maj, min_v);
#else
        int denoise = 0;
...
// Pick the DLSS quality preset whose NGX-advertised OPTIMAL render extent is closest
// to the requested render ratio. Dynamic ranges overlap heavily (for example Quality
// also accepts a 0.5 input), so choosing the first containing range silently maps
// Performance to Quality. After selecting by optimum, validate that the exact input is
// inside that preset's NGX range. Fills h.sr_quality(_name) only on full success.
static bool PickSrQuality(UINT w, UINT h_, UINT out_w, UINT out_h)
{
    static const struct { NVSDK_NGX_PerfQuality_Value q; const char *name; } kModes[] = {
    ...
    bvr_sr_quality::Candidate candidates[_countof(kModes)] = {};
    for (size_t i = 0; i < _countof(kModes); ++i)
    {
        const auto &o = kModes[i];
        auto &candidate = candidates[i];
        candidate.quality = static_cast<int>(o.q);
        ...
        candidate.optimalWidth = opt_w;
        candidate.optimalHeight = opt_h;
        candidate.minimumWidth = min_w;
        candidate.minimumHeight = min_h;
        candidate.maximumWidth = max_w;
        candidate.maximumHeight = max_h;
        candidate.offered = true;
    }

    bvr_sr_quality::Selection selection{};
    if (!bvr_sr_quality::closest_optimal(candidates, _countof(candidates), w, h_,
                                         out_w, out_h, &selection))
    {
        Log("[host] no DLSS SR preset advertises an optimum at %ux%u", out_w, out_h);
        return false;
    }

    const auto &chosen = candidates[selection.index];
    Log("[host] DLSS SR ratio %.4fx%.4f selects %s by nearest NGX optimum %ux%u",
        static_cast<double>(w) / out_w, static_cast<double>(h_) / out_h,
        chosen.name, chosen.optimalWidth, chosen.optimalHeight);
    if (!bvr_sr_quality::input_in_range(chosen, w, h_))
    {
        Log("[host] DLSS %s selected by optimum rejects render %ux%u; NGX range is "
            "%ux%u .. %ux%u",
            chosen.name, w, h_, chosen.minimumWidth, chosen.minimumHeight,
            chosen.maximumWidth, chosen.maximumHeight);
        return false;
    }

    h.sr_quality = chosen.quality;
    h.sr_quality_name = chosen.name;
    return true;
}

// DLSS 4.5 recommended model mapping (SDK 310.7): K is the high-quality
// transformer used for DLAA/Quality/Balanced, M is the Performance model and
// L is the Ultra Performance model.  Set every hint explicitly so this helper
// never inherits an NVIDIA App override or an old DLSS Swapper experiment.
static void ApplyDlss45RecommendedPresets()
{
    if (h.params == nullptr) return;
    h.params->Set(NVSDK_NGX_Parameter_DLSS_Hint_Render_Preset_DLAA,
                  static_cast<int>(NVSDK_NGX_DLSS_Hint_Render_Preset_K));
    h.params->Set(NVSDK_NGX_Parameter_DLSS_Hint_Render_Preset_Quality,
                  static_cast<int>(NVSDK_NGX_DLSS_Hint_Render_Preset_K));
    h.params->Set(NVSDK_NGX_Parameter_DLSS_Hint_Render_Preset_Balanced,
                  static_cast<int>(NVSDK_NGX_DLSS_Hint_Render_Preset_K));
    h.params->Set(NVSDK_NGX_Parameter_DLSS_Hint_Render_Preset_Performance,
                  static_cast<int>(NVSDK_NGX_DLSS_Hint_Render_Preset_M));
    h.params->Set(NVSDK_NGX_Parameter_DLSS_Hint_Render_Preset_UltraPerformance,
                  static_cast<int>(NVSDK_NGX_DLSS_Hint_Render_Preset_L));
    h.params->Set(NVSDK_NGX_Parameter_DLSS_Hint_Render_Preset_UltraQuality,
                  static_cast<int>(NVSDK_NGX_DLSS_Hint_Render_Preset_K));
}
...
// Audit the modules after NGX has created the feature: that is the first point
// where nvngx_dlss.dll is guaranteed to have been loaded.  The clean package
// may use driver NGX modules and the system DXGI/D3D12 DLLs, but never a local
// proxy, ReShade/RenoDX add-on, or an NR/Ray-Reconstruction feature DLL.
static bool AuditBvrDlss45Modules()
{
    wchar_t module_dir[MAX_PATH] = {};
    GetModuleFileNameW(nullptr, module_dir, MAX_PATH);
    if (wchar_t *slash = wcsrchr(module_dir, L'\\')) *(slash + 1) = L'\0';

    wchar_t expected_dlss[MAX_PATH] = {};
    _snwprintf_s(expected_dlss, _TRUNCATE, L"%snvngx_dlss.dll", module_dir);

    HANDLE snapshot = CreateToolhelp32Snapshot(TH32CS_SNAPMODULE, GetCurrentProcessId());
    if (snapshot == INVALID_HANDLE_VALUE)
    {
        Log("[host] DLSS45 module audit FAILED: CreateToolhelp32Snapshot error %lu", GetLastError());
        return false;
    }
    ...
    if (!Module32FirstW(snapshot, &entry)) clean = false;
    else do
    {
        const wchar_t *name = entry.szModule;
        const wchar_t *path = entry.szExePath;
        if (_wcsicmp(name, L"nvngx_dlss.dll") == 0)
        {
            saw_dlss = true;
            if (_wcsicmp(path, expected_dlss) != 0)
            {
                Log("[host] DLSS45 module audit FAILED: nvngx_dlss.dll came from %ls (expected %ls)",
                    path, expected_dlss);
                clean = false;
            }
        }

        const bool forbidden_feature =
            _wcsicmp(name, L"nvngx_dlssnr.dll") == 0 ||
            _wcsicmp(name, L"nvngx_dlssd.dll") == 0;
        const bool forbidden_addon =
            ContainsInsensitive(name, L"renodx") ||
            ContainsInsensitive(name, L"reshade") ||
            ContainsInsensitive(name, L"dlss5-feed") ||
            ContainsInsensitive(name, L".addon64");
        const bool local_proxy =
            (_wcsicmp(name, L"dxgi.dll") == 0 || _wcsicmp(name, L"d3d11.dll") == 0) &&
            _wcsnicmp(path, module_dir, wcslen(module_dir)) == 0;
        if (forbidden_feature || forbidden_addon || local_proxy)
        {
            Log("[host] DLSS45 module audit FAILED: forbidden module %ls -> %ls", name, path);
            clean = false;
        }
    ...
    if (clean)
        Log("[host] DLSS45 module audit PASS: validated runtime candidate=%ls; resident nvngx_dlss.dll=%s; "
            "forbidden DLSS5/NR/RenoDX/ReShade/local-proxy modules=0",
            expected_dlss, saw_dlss ? "yes (loader-listed)" : "not loader-listed (NGX-managed)");
    return clean;
}
#endif
...
#endif
    if (sr) Log("[host] feature ready: %ux%u -> %ux%u DLSS 4.5 %s (model K/M/L recommended mapping) flags=%d", w, h_, target_w, target_h, h.sr_quality_name, flags);
    else    Log("[host] feature ready: %ux%u DLAA 4.5 (model K) flags=%d", w, h_, flags);
    return true;
}
                     ...
                     UINT w, UINT h_, int reset, float mvsx, float mvsy, float jitter_x = 0.0f, float jitter_y = 0.0f)
{
    ...
    // Render-pixel camera jitter. The BioShock client currently forwards zero
    // for both SR and DLAA until its raster projection is shifted too.
    ep.InJitterOffsetX   = jitter_x;
...
static int RunTest(bool sr)
{
    const UINT W = sr ? 960 : 640, H = sr ? 540 : 360;
    const UINT OUT_W = sr ? 1920 : W, OUT_H = sr ? 1080 : H;
    Log("[host] %s: %ux%u -> %ux%u synthetic %s 4.5", sr ? "--test-sr" : "--test",
        W, H, OUT_W, OUT_H, sr ? "DLSS Super Resolution" : "DLAA");
    if (sr && !PickSrQuality(W, H, OUT_W, OUT_H))
    {
        Log("[host] --test-sr: no DLSS quality mode accepts this render/output ratio");
        return 1;
    }
    if (!sr)
    {
    ...
    ID3D12Resource *output = MakeTex(OUT_W, OUT_H, DXGI_FORMAT_R8G8B8A8_UNORM, true);
    ...
    // Mirror BioShock Remastered's finite, conventional D3D depth path. The
    // production bridge supplies the same convention through FeedBuild.
    int flags = NVSDK_NGX_DLSS_Feature_Flags_MVLowRes |
                NVSDK_NGX_DLSS_Feature_Flags_AutoExposure;
    NVSDK_NGX_Result rf = NVSDK_NGX_Result_Fail;
    if (!CreateFeature(W, H, flags, &rf, sr ? OUT_W : 0, sr ? OUT_H : 0)) return 1;

    int good = 0;
    for (int i = 0; i < 300; ++i)
    {
        PumpPresent(true);
        // A tiny deterministic two-sample jitter proves that the SR path accepts
        // temporal offsets. The game integration will replace this with its per-eye sequence.
        const float jx = sr ? ((i & 1) ? 0.25f : -0.25f) : 0.0f;
        const float jy = sr ? ((i & 2) ? 0.25f : -0.25f) : 0.0f;
        if (Evaluate(color, output, depth, mv, W, H, i == 0 ? 1 : 0, 1.0f, 1.0f, jx, jy)) ++good;
        else break;
        if (i == 180)   // the warm-up re-create, same medicine as in-game
        {
            ...
            h.feature = nullptr;
            if (!CreateFeature(W, H, flags, &rf, sr ? OUT_W : 0, sr ? OUT_H : 0)) { h.feature = old; Log("[host] keeping the previous feature"); }
    ...
    if (h.ts_n > 0)
        Log("[host] %s: DLSS GPU %.2f ms/frame over %u timed frames at %dx%d -> %dx%d",
            sr ? "--test-sr" : "--test", h.ts_sum_ms / double(h.ts_n), h.ts_n, W, H, OUT_W, OUT_H);
    else
...
static int Serve(DWORD game_pid, int eye)
{
    char name[128];
    if (eye == 0 || eye == 1)
        sprintf_s(name, "\\\\.\\pipe\\bvr-dlss45.%lu.eye%d",
                  static_cast<unsigned long>(game_pid), eye);
    else
    ...
    UINT64 hold_until = g_bvr_dlss45_mode ? 0 : GetTickCount64() + 800;
    UINT64 evaluated  = 0;
    bool   warm_done  = g_bvr_dlss45_mode || g_renodx_lazy;
    int    build_fails = 0;
    ...
    for (;;)
    {
        ...
        if (tag == 'B')
        {
            ...
            warm_done = g_bvr_dlss45_mode || transport_only || g_renodx_lazy ||
                        (g_chicken_present && !g_chicken_created_unarmed);
...
static void ShutdownDisguise()
{
    ...
    Log(g_bvr_dlss45_mode ? "[host] BioShock VR D3D12 helper released"
                          : "[host] disguise released (ReShade saves its ini here)");
}
...
int main(int argc, char **argv)
{
...
#if defined(BVR_DLSS45_ONLY)
        MessageBoxW(nullptr,
                    L"BioShock VR DLSS 4.5 Complemento v1.0.0-rc.1 \u00B7 By Beren5556\r\n\r\n"
                    L"Este componente de 64 bits se inicia autom\u00E1ticamente desde el mod VR.\r\n"
                    L"No hay que ejecutarlo directamente.",
                    L"BioShock VR DLSS 4.5 \u00B7 By Beren5556", MB_OK | MB_ICONINFORMATION);
#else
        MessageBoxA(nullptr,
    ...
    // Parse the BioShock eye selector before choosing the log path. The full
    // parser below still validates it and handles every other argument.
    for (int i = 1; i + 1 < argc; ++i)
    {
        if (strcmp(argv[i], "--eye") == 0)
        {
            const int candidate = atoi(argv[i + 1]);
            if (candidate == 0 || candidate == 1)
            {
                g_bvr_dlss45_mode = true;
                g_bvr_eye = candidate;
            }
            break;
        }
...
#if defined(BVR_DLSS45_ONLY)
    // The packaged BioShock VR helper must never fall through to the feeder's
    // legacy ReShade/RenoDX lane.  The source is intentionally shared, but this
    // build accepts only the two explicit stereo lanes selected by --eye.
    if (!g_bvr_dlss45_mode)
        return 2;
#endif
    ...
    if (char *s = strrchr(g_log_path, '\\'))
        strcpy_s(s + 1, MAX_PATH - (s + 1 - g_log_path),
                 g_bvr_dlss45_mode
                     ? (g_bvr_eye == 0 ? "BioShockVR-DLSS45-eye0.log"
                                       : "BioShockVR-DLSS45-eye1.log")
                     : "dlss5-feed-host.log");
    ...
    // the path we settled on so nobody reads a stale file.
    {
        ...
        else
        {
            ...
            {
                strcat_s(fallback, MAX_PATH,
                         g_bvr_dlss45_mode ? "\\BioshockVR\\DLSS45" : "\\DLSS5-Feeder");
...
#if defined(BVR_DLSS45_ONLY)
    Log("%s (built %s %s)", kProductIdentity, __DATE__, __TIME__);
#else
    ...
    bool  test = false, test_sr = false, hide = false, behind = false;
    int   eye = -1;
    DWORD pid = 0;
    for (int i = 1; i < argc; ++i)
    {
        if      (strcmp(argv[i], "--test") == 0) test = true;
        else if (strcmp(argv[i], "--test-sr") == 0) { test = true; test_sr = true; }
        else if (strcmp(argv[i], "--eye") == 0 && i + 1 < argc)
        {
            eye = atoi(argv[++i]);
            if (eye != 0 && eye != 1)
            {
                Log("[host] invalid --eye value %d (expected 0 or 1)", eye);
                return 1;
            }
        }
    ...
    {
#if defined(BVR_DLSS45_ONLY)
        Log("uso interno: BioShockVR-DLSS45-Host64.exe <pid> --eye 0|1 --hide");
#else
        Log("usage: dlss5-feed-host64 --test | --test-sr | dlss5-feed-host64 <game pid> [--eye 0|1] [--hide | --behind]");
#endif
        return 1;
    }
#if defined(BVR_DLSS45_ONLY)
    if (test || eye < 0)
    {
        Log("[host] invocacion rechazada: esta edicion requiere un PID y --eye 0|1");
        return 2;
    }
    // No tuning window or legacy overlay is permitted in this package, even
    // if a caller omits --hide.
    hide = true;
    behind = false;
#endif
    ...
    if (!g_bvr_dlss45_mode)
    {
        ...
        Log("[host] BioShock VR dedicated lane: DLSS 4.5 SR/DLAA (eye %d)",
            g_bvr_eye);
    ...
    if (!InitDisguise())
        ...
        rc = test ? RunTest(test_sr) : Serve(pid, eye);
