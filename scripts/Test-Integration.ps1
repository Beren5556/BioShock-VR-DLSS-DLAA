...
if (-not $BuildDirectory) { $BuildDirectory = Join-Path $repo 'artifacts\integration-0.2.13\build\src\Release' }
$reportRoot = Join-Path $repo ('artifacts\integration-0.2.13\tests\' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
foreach ($name in $names) {
    ...
    if (-not (Test-Path -LiteralPath $exe -PathType Leaf)) { throw "Falta compilar $name" }
...
if (@($results | Where-Object { -not $_.passed }).Count) { throw 'Hay pruebas fallidas; consultar sus registros.' }
