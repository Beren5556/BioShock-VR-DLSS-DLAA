...
if (-not $OutputDirectory) { $OutputDirectory = Join-Path $repoRoot 'artifacts\integration-0.2.13\launcher' }
$compilerPath = Join-Path $env:WINDIR 'Microsoft.NET\Framework64\v4.0.30319\csc.exe'
if (-not (Test-Path -LiteralPath $compilerPath)) {
    $compilerPath = Join-Path $env:WINDIR 'Microsoft.NET\Framework\v4.0.30319\csc.exe'
}
if (-not (Test-Path -LiteralPath $compilerPath)) { throw 'No se encontró el compilador C# de .NET Framework.' }
...
foreach ($selectedGame in $games) {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) { throw "La compilación de $selectedGame terminó con código $LASTEXITCODE." }
        if ([Diagnostics.FileVersionInfo]::GetVersionInfo($temporaryExe).FileVersion -ne '0.2.13.0') {
            throw 'Versión de lanzador incorrecta.'
        }
