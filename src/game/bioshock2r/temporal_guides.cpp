...
struct CameraState {
    ...
    float farPlane = 0.0f;
    float renderedTanX = 0.0f;
    float renderedTanY = 0.0f;
    std::uint32_t projectionPublications = 0;
...
// Explain ineffective reuse without changing capture, voting or invalidation.
// Updated only on the immediate-context path under the existing exit gate.
struct ReuseAuditWindow {
    uint64_t readOnlyStates = 0, writableStates = 0;
    uint64_t indexedReadOnly = 0, indexedWritable = 0, otherWritable = 0;
    uint64_t clears = 0, resourceWrites = 0, contextResets = 0;
    uint64_t firstCopies = 0, winnerCopies = 0, dirtyCopies = 0;
} g_reuseAudit;
CaptureMetadata g_captureMetadata{};
Diagnostics g_diagnostics{};
UINT g_width = 0;
UINT g_height = 0;
float g_nearPlane = 0.0f;
...
bool g_loggedReject[kEyeCount][10] = {};
...
bool dsv_is_compatible(ID3D11DepthStencilView* dsv, ID3D11Texture2D* texture) {
    ...
    if (!viewFormat || !resourceFormat) {
        if (!g_loggedUnsupportedFormat) {
            BVR_LOG("[bs2-dlss45-guides] unsupported depth format (DSV %u, resource %u); "
    ...
    if (viewDesc.ViewDimension != D3D11_DSV_DIMENSION_TEXTURE2D ||
        ...
        if (!g_loggedUnsupportedView) {
            BVR_LOG("[bs2-dlss45-guides] ignored DSV view dimension=%u mip=%u "
    ...
    if (textureDesc.Width != g_width || textureDesc.Height != g_height ||
        ...
        if (!g_loggedUnsupportedShape) {
            BVR_LOG("[bs2-dlss45-guides] ignored depth shape %ux%u array=%u samples=%u "
...
Candidate* candidate_for(ID3D11DepthStencilView* dsv) {
    ...
    if (g_candidateCount == kMaxCandidates) {
        if (!g_loggedCandidateOverflow) {
            BVR_LOG("[bs2-dlss45-guides] more than %d compatible DSVs in one interval; "
...
void finish_current_pass(ID3D11DeviceContext* context) {
    ...
    if (g_current == g_best || g_current->draws > g_bestDraws) {
        ...
        if (unchanged) {
            ...
            if (!g_haveCapture) ++g_reuseAudit.firstCopies;
            else if (g_current != g_best) ++g_reuseAudit.winnerCopies;
            else ++g_reuseAudit.dirtyCopies;
...
bool compile_shader(ID3DBlob** bytecode) {
    ...
    const HRESULT hr = D3DCompile(kGuideShader, std::strlen(kGuideShader),
                                  "bioshock2r_temporal_guides.hlsl", nullptr, nullptr,
                                  "cs_main", "cs_5_0", D3DCOMPILE_ENABLE_STRICTNESS,
                                  0, bytecode, &errors);
    if (errors) {
        BVR_LOG("[bs2-dlss45-guides] shader compile: %s",
...
CameraState current_camera(int eye, const Projection& projection) {
    ...
    if (result.valid) {
        ...
        result.nearPlane = published.nearPlane;
        result.farPlane = published.farPlane;
        result.renderedTanX = published.tanHalfFovX;
        result.renderedTanY = published.tanHalfFovY;
        result.projectionPublications = published.projectionPublications;
        result.projectionValid = published.projectionValid;
        result.historyEpoch = published.historyEpoch;
    }
    return result;
}
...
void seed_projection_diagnostics(EyeDiagnostics& diag, const Projection& projection) {
    ...
    diag.lastNearPlane = 0.0f;
    diag.lastFarPlane = 0.0f;
    diag.lastProjectionPublications = 0;
    diag.lastHistoryEpoch = 0;
...
void reject_frame(int eye, RejectReason reason) {
    ...
    if (reasonIndex < 10 && !g_loggedReject[eye][reasonIndex]) {
...
bool prepare(ID3D11Device* device, const PrepareDesc& desc) {
    shutdown();
    if (!device || !desc.width || !desc.height || desc.depthInverted) {
        BVR_LOG("[bs2-dlss45-guides] invalid prepare: %ux%u near=%.4f far=%.4f",
    ...
    g_nearPlane = 0.0f;
    g_farPlane = 0.0f;
    g_depthInverted = false; // verified finite D3D matrix, near=0 and far=1
    g_device = device;
    g_device->AddRef();
    ...
    if (!ok) {
        BVR_LOG("[bs2-dlss45-guides] D3D11 resource/SRV creation failed; temporal guides disabled");
        shutdown();
        return false;
    }

    BVR_LOG("[bs2-dlss45-guides] ready %ux%u; finite near/far and projection required per eye Build",
            g_width, g_height);
...
void log_performance() {
    ...
    BVR_LOG("[bs2-depth-reuse-audit] stateReadOnly=%llu stateWritable=%llu "
            "indexedReadOnly=%llu indexedWritable=%llu otherWritable=%llu "
            "depthClears=%llu resourceWrites=%llu contextResets=%llu "
            "copiesFirst=%llu copiesNewWinner=%llu copiesDirty=%llu; observer-only",
            static_cast<unsigned long long>(g_reuseAudit.readOnlyStates),
            static_cast<unsigned long long>(g_reuseAudit.writableStates),
            static_cast<unsigned long long>(g_reuseAudit.indexedReadOnly),
            static_cast<unsigned long long>(g_reuseAudit.indexedWritable),
            static_cast<unsigned long long>(g_reuseAudit.otherWritable),
            static_cast<unsigned long long>(g_reuseAudit.clears),
            static_cast<unsigned long long>(g_reuseAudit.resourceWrites),
            static_cast<unsigned long long>(g_reuseAudit.contextResets),
            static_cast<unsigned long long>(g_reuseAudit.firstCopies),
            static_cast<unsigned long long>(g_reuseAudit.winnerCopies),
            static_cast<unsigned long long>(g_reuseAudit.dirtyCopies));
    g_reuseAudit = {};
}
...
void on_depth_state(ID3D11DeviceContext* context, ID3D11DepthStencilState* state) {
    if (!kDepthCopyReuse || !ready()) return;
    ...
    if (g_depthWritesEnabled) ++g_reuseAudit.writableStates;
    else ++g_reuseAudit.readOnlyStates;
}
...
void on_setrt(ID3D11DeviceContext* context, UINT, ID3D11RenderTargetView* const*,
    ...
    // Context hooks can run outside Present. Keep terminal shutdown from
    // freeing depth candidates after this callback has observed ready=true.
    // The first ready check keeps NORMAL/off at its former cheap early return.
    if (!ready()) return;
    game_exit::Scope runtimeScope(game_exit::host_is_bioshock2());
    if (!runtimeScope) return;
...
void on_draw_indexed(ID3D11DeviceContext* context) {
    ...
    if (g_depthWritesEnabled) ++g_reuseAudit.indexedWritable;
    else ++g_reuseAudit.indexedReadOnly;
...
bool generate_eye(ID3D11DeviceContext* context, int eye,
    ...
    if (g_current && g_currentPassDraws) {
        if (!g_loggedBoundFinalize) {
            BVR_LOG("[bs2-dlss45-guides] scene DSV remained bound at generate_eye; "
    ...
    diag.lastNearPlane = current.nearPlane;
    diag.lastFarPlane = current.farPlane;
    diag.lastProjectionPublications = current.projectionPublications;
    diag.lastHistoryEpoch = current.historyEpoch;
    ...
    if (!std::isfinite(current.location[0]) || !std::isfinite(current.location[1]) ||
        !std::isfinite(current.location[2])) {
        reject_frame(eye, RejectReason::MissingCamera);
        reset_interval();
        return false;
    }
    if (!current.projectionValid || !current.projectionPublications ||
        !std::isfinite(current.nearPlane) || !std::isfinite(current.farPlane) ||
        current.nearPlane <= 0.0f || current.farPlane <= current.nearPlane) {
        reject_frame(eye, RejectReason::MissingProjection);
        reset_interval();
        return false;
    }
    if (current.projectionPublications != 1) {
        reject_frame(eye, RejectReason::AmbiguousProjection);
        reset_interval();
        return false;
    }
    Projection capturedProjection = projection;
    capturedProjection.observedValid = true;
    capturedProjection.observedTanHalfFovX = current.renderedTanX;
    capturedProjection.observedTanHalfFovY = current.renderedTanY;
    if (!projection_matches_observation(capturedProjection)) {
        reject_frame(eye, RejectReason::ProjectionMismatch);
        reset_interval();
        return false;
    }
    // Reconstruct from the matrix that this Build actually rendered. The
    // caller's compositor tangents are an independent agreement check above.
    current.tanX = current.renderedTanX;
    current.tanY = current.renderedTanY;
    g_nearPlane = current.nearPlane;
    g_farPlane = current.farPlane;
    ...
    const bool sameProjection =
        ...
        eyeState.previous.nearPlane == current.nearPlane &&
        eyeState.previous.farPlane == current.farPlane;
    ...
    for (int i = 0; i < 3; ++i) {
        const float delta = current.location[i] - eyeState.previous.location[i];
        translationSquared += delta * delta;
    }
    // A large camera cut/snap is safe to reconstruct as a fresh frame, never
    // as a long cross-world motion field. Thresholds are rejection policy,
    // not engine offsets or a claim about the game's movement speed.
    const bool cameraContinuous = translationSquared < 256.0f * 256.0f &&
        std::abs(temporal_contract::rotation_delta(current.rotation.yaw, eyeState.previous.rotation.yaw)) < 8192 &&
        std::abs(temporal_contract::rotation_delta(current.rotation.pitch, eyeState.previous.rotation.pitch)) < 8192 &&
        std::abs(temporal_contract::rotation_delta(current.rotation.roll, eyeState.previous.rotation.roll)) < 8192;
    const bool historyValid = sameProjection && sequenceContinuous && cameraContinuous &&
        current.historyEpoch == eyeState.previous.historyEpoch &&
        current.stampMs >= eyeState.previous.stampMs &&
        current.stampMs - eyeState.previous.stampMs <= 200;
...
const char* reject_reason_name(RejectReason reason) {
    ...
    case RejectReason::MissingProjection:  return "missing-projection";
    case RejectReason::AmbiguousProjection:return "ambiguous-projection";
    }
    return "unknown";
}
