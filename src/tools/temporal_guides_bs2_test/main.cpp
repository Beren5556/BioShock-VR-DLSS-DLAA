...
float nearPlane = 10.0f, farPlane = 65536.0f;
float tanX[2] = {1.0f, 1.0f}, tanY[2] = {0.75f, 0.75f};
std::uint32_t projectionPublications[2] = {1, 1};
bool projectionValid[2] = {true, true};
std::uint64_t historyEpoch = 1;
}
...
bool driven_eye_cam_for_build(int eye, std::uint64_t buildId, DrivenEyeCamera* out) {
    ...
    out->nearPlane = test_camera::nearPlane;
    out->farPlane = test_camera::farPlane;
    out->tanHalfFovX = test_camera::tanX[eye];
    out->tanHalfFovY = test_camera::tanY[eye];
    out->projectionPublications = test_camera::projectionPublications[eye];
    out->projectionValid = test_camera::projectionValid[eye];
    out->historyEpoch = test_camera::historyEpoch;
    return true;
}
} // namespace bvr::b2r::camera
...
void bind_color_depth(ID3D11DeviceContext* context, ID3D11RenderTargetView* rtv,
    ...
    bvr::b2r::temporal_guides::on_setrt(context, 1, &rtv, dsv);
}
...
void bind_depth(ID3D11DeviceContext* context, ID3D11DepthStencilView* dsv) {
    ...
    bvr::b2r::temporal_guides::on_setrt(context, 0, nullptr, dsv);
}

void vote(unsigned count, ID3D11DeviceContext* context) {
    for (unsigned i = 0; i < count; ++i)
        bvr::b2r::temporal_guides::on_draw_indexed(context);
}
...
void clear_depth(ID3D11DeviceContext* context, DepthSurface& surface, float depth) {
    ...
    bvr::b2r::temporal_guides::on_clear_dsv(
        context, surface.dsv, D3D11_CLEAR_DEPTH, depth, 0);
}

bvr::b2r::temporal_guides::Projection tagged_projection(
    int eye, std::uint64_t buildId, float tanX = 1.0f, float tanY = 0.75f) {
    test_camera::buildIds[eye] = buildId;
    test_camera::publications[eye] = 1;
    test_camera::tanX[eye] = tanX;
    test_camera::tanY[eye] = tanY;
    bvr::b2r::temporal_guides::Projection result{};
...
bool rotation_case(ID3D11Device* device, ID3D11DeviceContext* context,
    ...
    bvr::b2r::temporal_guides::invalidate_eye(0);
    ...
    bvr::b2r::temporal_guides::EyeGuides baseline{};
    ...
    bvr::b2r::temporal_guides::EyeGuides rotated{};
...
bool finite_depth_case(ID3D11Device* device, ID3D11DeviceContext* context,
                       ...
                       const char* name) {
    bvr::b2r::temporal_guides::PrepareDesc prepare{};
    prepare.width = width;
    prepare.height = height;
    // Deliberately wrong caller hints: exact captured arguments must win.
    prepare.nearPlane = 25.0f;
    prepare.farPlane = 4096.0f;
    test_camera::nearPlane = nearPlane;
    test_camera::farPlane = farPlane;
    prepare.depthInverted = inverted;
    if (!bvr::b2r::temporal_guides::prepare(device, prepare)) return false;
    ...
    bvr::b2r::temporal_guides::EyeGuides moved{};
...
namespace depth_reuse_target = bvr::b2r::temporal_guides;
#include "../temporal_guides_test/depth_reuse_cases.h"
...
int main() {
    ...
    ok = ok && bvr::b2r::temporal_guides::prepare(device, prepare);
    ...
    const bvr::b2r::temporal_guides::Projection projection =
        tagged_projection(0, 1);
    bvr::b2r::temporal_guides::EyeGuides left0{};
    if (ok) {
        ...
        vote(9, context);
        ok = bvr::b2r::temporal_guides::generate_eye(context, 0, projection, &left0);
    }
    if (ok) {
    ...
    bvr::b2r::temporal_guides::EyeGuides right0{};
    if (ok) {
    ...
    bvr::b2r::temporal_guides::EyeGuides leftAgain{};
    if (ok) {
        ok = bvr::b2r::temporal_guides::get_eye(0, &leftAgain) &&
    ...
    if (ok) {
        bvr::b2r::temporal_guides::Diagnostics diagnostics{};
        bvr::b2r::temporal_guides::get_diagnostics(&diagnostics);
    ...
    if (ok) {
        ...
        bvr::b2r::temporal_guides::EyeGuides fovReset{};
    ...
    if (ok) {
        capture_interval(context, minor, 0.20f, 1, main, 0.50f, 4);
        bvr::b2r::temporal_guides::Projection untagged{};
        untagged.tanHalfFovX = 1.0f;
        untagged.tanHalfFovY = 0.75f;
        bvr::b2r::temporal_guides::EyeGuides rejected{};
        bvr::b2r::temporal_guides::Diagnostics diagnostics{};
        ok = !bvr::b2r::temporal_guides::generate_eye(
            context, 0, untagged, &rejected);
        bvr::b2r::temporal_guides::get_diagnostics(&diagnostics);
        ok = ok && diagnostics.eyes[0].lastReject ==
                       bvr::b2r::temporal_guides::RejectReason::MissingBuildTag;
    ...
    if (ok) {
        ...
        bvr::b2r::temporal_guides::get_diagnostics(&diagnostics);
        ok = ok && diagnostics.eyes[0].lastReject ==
                       bvr::b2r::temporal_guides::RejectReason::MissingCamera;
    ...
    if (ok) {
        ...
        bvr::b2r::temporal_guides::get_diagnostics(&diagnostics);
        ok = ok && diagnostics.eyes[0].lastReject ==
                       bvr::b2r::temporal_guides::RejectReason::AmbiguousCamera;
    ...
    if (ok) {
        ...
        bvr::b2r::temporal_guides::get_diagnostics(&diagnostics);
        ok = ok && diagnostics.eyes[0].lastReject ==
                       bvr::b2r::temporal_guides::RejectReason::ProjectionMismatch;
    ...
    if (ok) {
        bvr::b2r::temporal_guides::EyeGuides seeded{}, gap{};
    ...
    if (ok) {
        ...
        bvr::b2r::temporal_guides::get_diagnostics(&diagnostics);
        ok = ok && diagnostics.eyes[0].lastReject ==
                       bvr::b2r::temporal_guides::RejectReason::OutOfOrderBuild;
    ...
    if (ok) {
        ...
        if (ok) {
            bvr::b2r::temporal_guides::on_setrt(deferred, 0, nullptr, main.dsv);
            bvr::b2r::temporal_guides::on_draw_indexed(deferred);
            bvr::b2r::temporal_guides::EyeGuides rejected{};
    ...
    // BS2's audited builder + far selector expose finite 1024/65536 branches.
    // Both use their actually published arguments, never PrepareDesc hints.
    if (ok)
    ...
    if (ok)
        ok = finite_depth_case(device, context, minor, main, kWidth, kHeight,
                               ...
                               "finite near=5");
    if (ok) {
        bvr::b2r::temporal_guides::EyeGuides before{}, after{};
        ok = generate_frame(context, minor, main, 0, 321, 0.50f, &before);
        ++test_camera::historyEpoch;
        ok = ok && generate_frame(context, minor, main, 0, 323, 0.50f, &after) &&
             after.resetRequired && !after.historyValid;
        if (!ok) std::printf("FAIL: mode/world/recenter epoch did not reset history\n");
    }
    if (ok) {
        bvr::b2r::temporal_guides::EyeGuides after{};
        test_camera::locations[0][0] += 1000.0f;
        ok = generate_frame(context, minor, main, 0, 325, 0.50f, &after) &&
             after.resetRequired && !after.historyValid;
        if (!ok) std::printf("FAIL: camera teleport did not reset history\n");
    }
    if (ok) {
        bvr::b2r::temporal_guides::EyeGuides after{};
        test_camera::farPlane = 65536.0f;
        ok = generate_frame(context, minor, main, 0, 327, 0.50f, &after) &&
             after.resetRequired && !after.historyValid;
        if (!ok) std::printf("FAIL: finite far-plane branch change did not reset history\n");
    }
    for (int mode = 0; ok && mode < 3; ++mode) {
        test_camera::projectionValid[0] = mode != 0;
        test_camera::projectionPublications[0] = mode == 1 ? 2u : 1u;
        capture_interval(context, minor, 0.20f, 1, main, 0.50f, 4);
        const auto projection = tagged_projection(0, 331 + 2 * mode);
        if (mode == 2) test_camera::tanX[0] = 1.30f;
        bvr::b2r::temporal_guides::EyeGuides rejected{};
        bvr::b2r::temporal_guides::Diagnostics diagnostics{};
        ok = !bvr::b2r::temporal_guides::generate_eye(context, 0, projection, &rejected);
        bvr::b2r::temporal_guides::get_diagnostics(&diagnostics);
        const auto expected = mode == 0
            ? bvr::b2r::temporal_guides::RejectReason::MissingProjection
            : mode == 1 ? bvr::b2r::temporal_guides::RejectReason::AmbiguousProjection
                        : bvr::b2r::temporal_guides::RejectReason::ProjectionMismatch;
        ok = ok && diagnostics.eyes[0].lastReject == expected;
        if (!ok) std::printf("FAIL: exact projection rejection case %d\n", mode);
    }
    test_camera::projectionValid[0] = true;
    test_camera::projectionPublications[0] = 1;

    // Validate real hook math independently from the GPU camera stub.
    if (ok) {
        using namespace bvr::b2r::temporal_contract;
        const float poseA[3] = {1.0f, -500.0f, 10000.0f};
        float poseB[3] = {1.0f, -500.0f, 10000.0f};
        const int32_t rotA[3] = {0, -1, INT32_MIN};
        int32_t rotB[3] = {65536, 65535, 0};
        ok = same_pose(poseA, rotA, poseB, rotB) &&
             rotation_delta(INT32_MIN, INT32_MAX) == 1 &&
             rotation_delta(65535, 0) == -1 &&
             rotation_delta(0, 65535) == 1;
        poseB[0] += 0.02f;
        ok = ok && !same_pose(poseA, rotA, poseB, rotB);
        poseB[0] = poseA[0];
        ++rotB[1];
        ok = ok && !same_pose(poseA, rotA, poseB, rotB);
        if (!ok) std::printf("FAIL: root scene/camera pose identity validation\n");
    }
    if (ok) {
        float matrix[16]{};
        matrix[0] = 1.0f;
        matrix[5] = 4.0f / 3.0f;
        matrix[10] = 1024.0f / 1014.0f;
        matrix[11] = 1.0f;
        matrix[14] = -10.0f * matrix[10];
        float tx = 0, ty = 0;
        ok = bvr::b2r::temporal_contract::finite_projection(matrix, 10, 1024, &tx, &ty) &&
             almost(tx, 1.0f) && almost(ty, 0.75f);
        matrix[14] = -matrix[14]; // reversed depth must be rejected
        ok = ok && !bvr::b2r::temporal_contract::finite_projection(matrix, 10, 1024, &tx, &ty);
        matrix[14] = -matrix[14];
        matrix[8] = 0.01f; // unexpected asymmetric/off-axis matrix
        ok = ok && !bvr::b2r::temporal_contract::finite_projection(matrix, 10, 1024, &tx, &ty);
        if (!ok) std::printf("FAIL: audited finite D3D projection validation\n");
    }

    if (ok) {
        bvr::b2r::temporal_guides::invalidate();
        bvr::b2r::temporal_guides::EyeGuides stale{};
        ok = !bvr::b2r::temporal_guides::get_eye(0, &stale) &&
             !bvr::b2r::temporal_guides::get_eye(1, &stale);
        if (!ok) std::printf("FAIL: invalidate retained a generated eye\n");
    }

#ifdef BVR_DEPTH_COPY_REUSE
    test_camera::nearPlane = 10.0f;
    test_camera::farPlane = 65536.0f;
    test_camera::projectionValid[0] = true;
    test_camera::projectionPublications[0] = 1;
    if (ok) ok = bvr::b2r::temporal_guides::prepare(device, prepare);
    ...
    bvr::b2r::temporal_guides::shutdown();
    ...
    std::printf("%s: BioShock 2 temporal-guides WARP test\n", ok ? "PASS" : "FAIL");
    return ok ? 0 : 2;
}
